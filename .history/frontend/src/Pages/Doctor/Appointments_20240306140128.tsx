...
const Appointments = () => {
  ...
  return doctor.appointments && doctor.appointments.length > 0 ? (
    <table className="text-center border-spacing-8 border-separate text-textColor">
      <thead>
        <tr className="">
          <th>Name</th>
          <th>Gender</th>
...
const AppointmentRow: React.FC<{ appoint: Appointment }> = ({ appoint }) => {
  return (
    <tr className="font-[500] text-purple-500 border-2 border-black">
      <td className="border">{appoint.user?.name}</td>
