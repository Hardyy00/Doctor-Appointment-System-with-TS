...
import { createReview, getReviews } from "../../util/http";
...
const DoctorFeedback: React.FC = () => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: async (data: { review: Review }) => {
      queryClient.invalidateQueries({ queryKey: ["doctor", id] });
      await queryClient.cancelQueries({ queryKey: ["doctor", "reviews", id] });

      const previousData: Review[] = queryClient.getQueryData([
        "doctor",
        "reviews",
        id,
      ])!;

      const newState = [...previousData, data.review];

      queryClient.setQueryData(["doctor", "reviews", id], newState);

      setOpen(false);
    },
  });

  const feedbackSubmit = (rating: number, feedback: string) => {
    mutate({ rating, feedback, userId, doctorId: id! });
  };
...
const GiveFeedback: React.FC<{
  feedbackSubmit: (rating: number, feedback: string) => void;
  isPending: boolean;
}> = ({ feedbackSubmit, isPending }) => {
  ...
  const submitHandler = () => {
    ...
    const feedback: string = feedbackRef.current?.value as string;

    if (feedbackRef.current) {
      feedbackRef.current.value = "";
    }

    feedbackSubmit(rating, feedback);
  };
  return (
    <div className="mt-[3rem] flex flex-col w-full gap-6">
      ...
      <button className="btn w-[10rem] py-3" onClick={submitHandler}>
        {isPending && <ClipLoader size={28} color="white" />}
        {!isPending && "Share"}
      </button>
    </div>
  );
