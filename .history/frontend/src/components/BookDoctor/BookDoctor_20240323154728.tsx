...
const BookDoctor: React.FC<{ doctor: Doctor; isBooked: boolean }> = ({
  doctor,
  isBooked,
}) => {
      ...
      handler: async (response) => {
        ...
        if (status == 200) {
          ...
          mutate({
            doctor: doctor._id!,
            user: user._id!,
            ticketPrice: +order.amount as number,
    ...
    <div className="px-[1rem] w-[22rem] py-[2rem] h-auto box-decoration-slice text-textColor flex flex-col gap-[2rem] bg-white shadow-xl rounded-2xl">
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <Card className="flex flex-col items-center justify-center gap-8">
          ...
          <h2 className="font-[600] text-[1.1rem]">
            You Booked Doctor{" "}
            <span className="text-green-300 font-[800]"> {doctor?.name}</span>
          </h2>
      ...
      <div className="flex flex-col items-start justify-between gap-[1rem]">
        ...
        <div className="flex flex-col w-full gap-[1rem]">
          {doctor.timeSlots?.map((item, index: number) => {
            return <Slot slot={item} key={index} />;
          })}
        </div>
      </div>
      ...
      <div>
        <input
          ...
          className="hidden"
          ref={dateRef}
          onChange={doctorBookHandler}
        />
      </div>

      <button
        className={`btn p-[1rem] rounded-md`}
        style={{ backgroundColor: !user || isBooked ? "gray" : "" }}
