  ...
  const doctorBookHandler = async () => {
    ...
    const options = {
      ...
      handler: (response)=>{
          ...
          const verifyPaymentUrl = import.meta.env.VITE_BASE_URL + "/api/verifyPayment";
        const {data : {success}} = axios.post(verifyPaymentUrl,{razorpay_order_id,razorpay_payment_id,razorpay_signature})
      }
      prefill: {
        userId: user._id,
