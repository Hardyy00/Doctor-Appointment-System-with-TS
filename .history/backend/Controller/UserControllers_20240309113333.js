...
exports.getReviews = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ reviews: JSON.stringify(doctor.reviews) });
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
};
...
exports.getDoctorInformation = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ doctor: JSON.stringify(restDoctor) });
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
};
...
exports.getDoctors = async (req, res) => {
    ...
    try{
      ...
      const doctors = await Doctor.find().select('name image hospital specialization averageRating');

      const foundDoctors = doctors.map((item)=> item.name.includes(name));
    }
};
