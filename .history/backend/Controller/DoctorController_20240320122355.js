...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (comp) {
      const { appointments, updatedAt, ...rest } = doctor._doc;
...
exports.createAppointment = async (req, res) => {
  const { ticketPrice, user, doctor, appointmentDate, timeSlot } = req.body;
...
exports.getAppointments = async (req, res) => {
  ...
  try {
    ...
    appointments = appointments.map((item) => {
      ...
      const { password, updatedAt, ...restUser } = user._doc;

      restBook.user = restUser;

      delete restBook.doctor;
